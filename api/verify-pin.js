...
module.exports = function handler(req, res) {
  ...
  if (!limit.ok) return res.status(429).json({ error: "Muitas tentativas. Aguarde " + limit.restante + " segundos." });
  ...
  const pinCorreto = tipo === "admin" ? process.env.ADMIN_PIN : process.env.APP_PIN;
