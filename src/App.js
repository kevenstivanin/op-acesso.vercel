...
// ── Helpers de criptografia via API serverless ───────────────────────────────
async function criptografarCPF(cpf, pin) {
  const res = await fetch("/api/encrypt", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ cpf }),
  });
  if (!res.ok) throw new Error("Erro ao criptografar CPF");
  const { encrypted } = await res.json();
  return encrypted;
}

async function descriptografarCPF(encrypted, adminPin) {
  ...
  if (!res.ok) {
    ...
    throw new Error(error || "Erro ao descriptografar");
  }
...
const MOTIVOS_LIBERACAO = ["Criança","Sócio inadimplente","Transferência","Setor incorreto","Acesso já utilizado","Responsável já acessou","Catraca com defeito","Outros"];
...
function MotivoBadge({motivo}){
  const paleta={
    ...
    "Catraca com defeito":"#EFEBE9,#4E342E","Outros":"#F1F5F9,#334155",
  };
    ...
    <div style={{background:G.branco,borderRadius:10,border:"1.5px solid "+G.cinzaBorda,padding:"14px 16px",marginBottom:8,display:"flex",gap:12,alignItems:"flex-start",boxShadow:"0 1px 6px rgba(0,85,165,0.07)"}}>
      ...
      <div style={{flex:1,minWidth:0}}>
        ...
        <div style={{fontSize:13,color:"#1a1a1a",marginBottom:2}}>
          <span style={{fontWeight:700,color:G.azulEscuro}}>CPF:</span>{" "}
          <span style={{fontFamily:"monospace",background:"#f1f5f9",padding:"1px 6px",borderRadius:4,fontSize:12}}>
            {r.cpfMascarado||"***.***.***-**"}
          </span>
...
function TelaLogin({onLogin}){
  ...
  const handleLogin=async()=>{
    ...
    setTentando(true);
    try{
      const ok = await verificarPin(pin,"app");
      if(ok){
        sessionStorage.setItem("op_auth","1");
        onLogin();
      } else {
  ...
  return(
    <div style={{minHeight:"100vh",background:"linear-gradient(160deg,"+G.preto+" 0%,"+G.azulEscuro+" 50%,"+G.azul+" 100%)",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:24,fontFamily:"'Barlow','Segoe UI',sans-serif"}}>
      ...
      <div style={{width:"100%",maxWidth:360,textAlign:"center"}}>
        ...
        <div style={{marginTop:20,fontSize:10,color:"#334155",fontWeight:600}}>Sistema protegido · Acesso monitorado</div>
      </div>
    </div>
  );
}
...
function ModalAdminPin({onConfirm,onCancel}){
  ...
  const handleConfirm=async()=>{
    ...
    try{
      ...
      if(ok){ onConfirm(pin); }
      else { setErro("PIN de administrador incorreto."); setPin(""); setTimeout(()=>setErro(""),3000); }
    } catch(e){ setErro(e.message||"Erro de conexão."); }
    setTentando(false);
  };
  ...
  return(
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.7)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:1000,padding:20}}>
      <div style={{background:G.branco,borderRadius:16,padding:"28px 24px",width:"100%",maxWidth:340,boxShadow:"0 20px 60px rgba(0,0,0,0.4)"}}>
        <div style={{fontSize:16,fontWeight:900,color:G.azulEscuro,marginBottom:6,fontFamily:"'Barlow Condensed',sans-serif",textTransform:"uppercase",letterSpacing:0.5}}>🔐 Acesso de Administrador</div>
        <div style={{fontSize:12,color:G.cinzaTexto,marginBottom:20}}>Esta ação exportará CPFs descriptografados. Digite o PIN de administrador para continuar. A exportação será registrada em log.</div>
        ...
        <div style={{display:"flex",gap:10,marginTop:8}}>
          ...
          <button onClick={handleConfirm} disabled={tentando||!pin.trim()}
            style={{flex:1,padding:"11px",borderRadius:8,border:"none",background:G.azul,color:G.branco,cursor:"pointer",fontWeight:900,fontFamily:"'Barlow Condensed',sans-serif",textTransform:"uppercase",letterSpacing:0.5}}>
...
export default function App(){
  ...
  const [form,setForm]=useState({motivo:"",outrosDesc:"",portao:"",lider:liderAtual,cpf:""});
  ...
  const [appPin,setAppPin]=useState(sessionStorage.getItem("op_pin")||"");
  ...
  const carregarRegistros=useCallback(async()=>{
    ...
    // Exibe CPF mascarado — sem descriptografar na listagem
    setRegistros(data.map(r=>({
      id:r.id, portao:r.portao, lider:r.lider, hora:r.hora, data:r.data,
      motivo:r.motivo, cpfCriptografado:r.cpf,
      cpfMascarado:"***.***.***-**", // sempre mascarado na interface
    })));
    setLoading(false);
  },[]);
    ...
    const channel=supabase.channel("op-rt")
      .on("postgres_changes",{event:"INSERT",schema:"public",table:"atendimentos"},payload=>{
        ...
        setRegistros(prev=>prev.find(x=>x.id===r.id)?prev:[{
          id:r.id,portao:r.portao,lider:r.lider,hora:r.hora,data:r.data,
          motivo:r.motivo,cpfCriptografado:r.cpf,cpfMascarado:"***.***.***-**",
        },...prev]);
      })
  ...
  const handleLogin=()=>{
    // Salva o PIN na sessão para usar na criptografia
    const pinDigitado=sessionStorage.getItem("op_pin_temp")||"";
    setAppPin(pinDigitado);
    sessionStorage.setItem("op_auth","1");
    setAutenticado(true);
  };
  ...
  const handleSubmit=async()=>{
    ...
    try{
      ...
      const cpfCriptografado=await criptografarCPF(form.cpf, appPin);
      const {error}=await supabase.from("atendimentos").insert([{
        cpf:cpfCriptografado, motivo:motivoFinal, portao:form.portao, lider:form.lider, hora, data
      }]);
      if(error) throw new Error(error.message);
      setForm(f=>({...f,motivo:"",outrosDesc:"",cpf:""}));
      setSuccess(true); setTimeout(()=>setSuccess(false),2800);
    } catch(err){
      setDbErro("Erro ao salvar: "+err.message);
    }
    setSalvando(false);
  };
  ...
  const handleExportarComAdmin=async(adminPin)=>{
    ...
    setExportando(true);
    try{
      // Descriptografa todos os CPFs usando o servidor
      const cpfsDescriptografados=await Promise.all(
        registros.map(async r=>{
          try{ return await descriptografarCPF(r.cpfCriptografado,adminPin); }
          catch(e){ return "ERRO"; }
        })
      );
      ...
      const ws1=XLSX.utils.json_to_sheet(registros.map((r,i)=>({
        "#":i+1,"Data":r.data,"Hora":r.hora,
        "CPF do Torcedor":cpfsDescriptografados[i],
        "Motivo da Liberação":r.motivo,"Portão":r.portao,"Líder Responsável":r.lider,
      })));
      ws1["!cols"]=[{wch:4},{wch:12},{wch:8},{wch:18},{wch:30},{wch:22},{wch:20}];
      ...
      setDbErro("Erro ao exportar: "+err.message);
    }
    setExportando(false);
  };
  ...
  const filtered=useMemo(()=>{
    ...
    return registros.filter(r=>r.motivo.toLowerCase().includes(q)||r.portao.toLowerCase().includes(q)||r.lider.toLowerCase().includes(q));
  },[registros,filtro]);
      ...
      <div style={{background:"linear-gradient(135deg,"+G.preto+" 0%,"+G.azulEscuro+" 40%,"+G.azul+" 100%)",padding:"0 20px",boxShadow:"0 4px 20px rgba(0,0,0,0.4)"}}>
        ...
        <div style={{maxWidth:680,margin:"0 auto"}}>
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"14px 0 10px"}}>
            ...
            <button onClick={()=>{sessionStorage.clear();setAutenticado(false);}} style={{background:"rgba(255,255,255,0.1)",border:"1px solid rgba(255,255,255,0.2)",color:"#94BDDF",borderRadius:8,padding:"6px 12px",cursor:"pointer",fontSize:12,fontWeight:700,fontFamily:"inherit"}}>🔒 Sair</button>
          </div>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14}}>
              ...
              <div>
                ...
                <input type="text" placeholder="000.000.000-00" value={form.cpf}
                  ...
                  style={{...inp(errors.cpf),fontFamily:"'DM Mono',monospace",letterSpacing:1.2,backgroundImage:"none"}}/>
                {errors.cpf&&<div style={errStyle}>{errors.cpf}</div>}
              </div>
            </div>
            <div style={{marginTop:14,padding:"8px 12px",background:"#EFF6FF",borderRadius:7,border:"1px solid #BFDBFE",fontSize:11,color:"#1E40AF",display:"flex",gap:6}}>
              <span>🔐</span><span>CPF criptografado no servidor antes de salvar · LGPD</span>
            </div>
            <button onClick={handleSubmit} disabled={salvando}
              style={{width:"100%",marginTop:14,padding:"13px",background:salvando?"#94a3b8":"linear-gradient(135deg,"+G.azulEscuro+","+G.azul+")",color:G.branco,border:"none",borderRadius:9,fontSize:15,fontWeight:900,cursor:salvando?"not-allowed":"pointer",letterSpacing:0.8,fontFamily:"'Barlow Condensed',sans-serif",textTransform:"uppercase",boxShadow:"0 3px 12px rgba(0,85,165,0.35)"}}>
              {salvando?"⏳ Criptografando e salvando...":"⚽ Registrar Atendimento"}
            </button>
          </div>
        )}
        ...
        {tab==="lista"&&(
          <div style={{animation:"fadeIn 0.3s ease"}}>
            <div style={{display:"flex",gap:10,marginBottom:14,alignItems:"center"}}>
              <input type="text" placeholder="🔍  Buscar por motivo, portão ou líder..." value={filtro}
            ...
            <div style={{fontSize:11,color:G.cinzaTexto,marginBottom:12,background:"#F8FAFC",border:"1px solid "+G.cinzaBorda,borderRadius:7,padding:"7px 12px",display:"flex",gap:6,alignItems:"center"}}>
              🔐 <span>CPFs exibidos mascarados por proteção LGPD. Acesso completo apenas via exportação com PIN de administrador.</span>
            </div>
            {loading?(
            ...
            <div style={{background:G.branco,borderRadius:12,border:"1.5px solid "+G.cinzaBorda,padding:"18px 20px",marginBottom:14,boxShadow:"0 2px 12px rgba(0,85,165,0.09)"}}>
              <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:12}}>
                <div>
                  ...
                  <div style={{fontSize:12,color:G.cinzaTexto,marginTop:3}}>{registros.length} registro{registros.length!==1?"s":""} · Requer PIN de administrador</div>
                </div>
              ...
              <div style={{marginTop:12,fontSize:11,color:"#92400E",background:"#FEF3C7",border:"1px solid #FDE68A",borderRadius:7,padding:"8px 12px"}}>
                ⚠️ A exportação inclui CPFs descriptografados e será registrada em log com data, hora e responsável.
              </div>
            </div>
