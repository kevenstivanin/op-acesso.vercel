...
const MOTIVOS_LIBERACAO = [
  ...
  "Acesso já utilizado","Responsável já acessou","Catraca com defeito","Cortesia","Outros"
];
...
const MOTIVO_DESC = {
  ...
  "Acesso já utilizado":  { label:"Observação", placeholder:"Descreva se necessário...", obrigatorio:false },
  "Responsável já acessou":{ label:"Observação", placeholder:"Descreva se necessário...", obrigatorio:false },
  "Catraca com defeito":  { label:"Observação", placeholder:"Descreva se necessário...", obrigatorio:false },
  "Cortesia":             { label:"Observação", placeholder:"Descreva se necessário...", obrigatorio:false },
};
    ...
    <div style={{background:G.branco,borderRadius:10,border:"1.5px solid "+G.cinzaBorda,padding:"14px 16px",marginBottom:8,display:"flex",gap:12,alignItems:"flex-start",boxShadow:"0 1px 6px rgba(0,85,165,0.07)"}}>
      ...
      <div style={{flex:1,minWidth:0}}>
        <div style={{display:"flex",alignItems:"center",gap:8,flexWrap:"wrap",marginBottom:5}}>
          ...
          {r.origem==="atendente"?<span style={{fontSize:10,fontWeight:800,color:"#0369A1",background:"#E0F2FE",border:"1px solid #BAE6FD",padding:"1px 7px",borderRadius:99}}>📡 Atendente</span>:<span style={{fontSize:10,fontWeight:800,color:"#166534",background:"#DCFCE7",border:"1px solid #86EFAC",padding:"1px 7px",borderRadius:99}}>👤 Líder</span>}
...
export default function App(){
  ...
  const [form,setForm]=useState({motivo:"",outrosDesc:"",portao:"",lider:liderAtual,doc:"",tipoDoc:"cpf",origem:"lider",descExtra:""});
  ...
  const carregarRegistros=useCallback(async()=>{
    setLoading(true);
    const {data,error}=await supabase.from("atendimentos").select("id,portao,lider,hora,data,motivo,cpf,doc_extra,origem").order("created_at",{ascending:false});
    ...
    setRegistros(data.map(r=>({
      ...
      motivo:r.motivo,docCriptografado:r.cpf,docMascarado:"****",origem:r.origem||"lider",docExtra:r.doc_extra||null,
    })));
    setLoading(false);
  },[]);
  ...
  const validate=()=>{
    ...
    const mDescV=MOTIVO_DESC[form.motivo];
    if(mDescV&&mDescV.obrigatorio&&!form.descExtra.trim()) e.descExtra=mDescV.label+" é obrigatório";
  ...
  const handleSubmit=async()=>{
    ...
    try{
      ...
      const motivosSensiveis=["Criança","Transferência"];
      const isDocSensivel=motivosSensiveis.includes(form.motivo);
      const docExtraCriptografado=isDocSensivel&&form.descExtra.trim()
        ? await criptografarDoc(form.descExtra.trim())
        : null;
      const motivoComDesc=!isDocSensivel&&form.descExtra.trim()
        ? motivoFinal+" | "+form.descExtra.trim()
        : motivoFinal;
      const {error}=await supabase.from("atendimentos").insert([{
        cpf:docCriptografado,motivo:motivoComDesc,doc_extra:docExtraCriptografado,portao:form.portao,lider:form.lider,hora,data,origem:form.origem
      }]);
      if(error) throw new Error(error.message);
      setForm(f=>({...f,motivo:"",outrosDesc:"",doc:"",tipoDoc:"cpf",origem:"lider",descExtra:""}));
  ...
  const handleExportarComAdmin=async(adminPin)=>{
    ...
    setExportando(true);
    try{
      // Filtra por período se datas foram selecionadas
      const parseBR = (s) => { const [d,m,y]=s.split("/"); return new Date(+y,+m-1,+d); };
      const registrosFiltrados = registros.filter(r=>{
        if(!dataInicio && !dataFim) return true;
        const rd = parseBR(r.data);
        if(dataInicio && rd < new Date(dataInicio)) return false;
        if(dataFim && rd > new Date(dataFim)) return false;
        return true;
      });
      const docsDescriptografados=await Promise.all(
        registrosFiltrados.map(async r=>{
          try{ return await descriptografarDoc(r.docCriptografado,adminPin); }
          catch(e){ return "ERRO"; }
        })
      );
      const docsExtraDescriptografados=await Promise.all(
        registrosFiltrados.map(async r=>{
          if(!r.docExtra) return "";
          try{ return await descriptografarDoc(r.docExtra,adminPin); }
          catch(e){ return "ERRO"; }
        })
      );
      await registrarLog(adminPin,liderAtual||"admin",registrosFiltrados.length);
      const wb=XLSX.utils.book_new();
      const ws1=XLSX.utils.json_to_sheet(registrosFiltrados.map((r,i)=>({
        "#":i+1,"Data":r.data,"Hora":r.hora,"DOC":docsDescriptografados[i],"DOC Secundário":docsExtraDescriptografados[i]||"","Origem":r.origem==="atendente"?"Atendente":"Líder",
        "Motivo":r.motivo,"Portão":r.portao,"Líder":r.lider,
      })));
      ws1["!cols"]=[{wch:4},{wch:12},{wch:8},{wch:20},{wch:12},{wch:30},{wch:22},{wch:20}];
      XLSX.utils.book_append_sheet(wb,ws1,"Registros");
      const porMotivo=registrosFiltrados.reduce((acc,r)=>{const k=r.motivo.startsWith("Outros:")?"Outros":r.motivo;acc[k]=(acc[k]||0)+1;return acc;},{});
      const ws2=XLSX.utils.json_to_sheet([...Object.entries(porMotivo).sort((a,b)=>b[1]-a[1]).map(([k,v])=>({"Motivo":k,"Qtd":v})),{"Motivo":"TOTAL","Qtd":registrosFiltrados.length}]);
      XLSX.utils.book_append_sheet(wb,ws2,"Por Motivo");
      const porPortao=registrosFiltrados.reduce((acc,r)=>{acc[r.portao]=(acc[r.portao]||0)+1;return acc;},{});
      const ws3=XLSX.utils.json_to_sheet([...Object.entries(porPortao).sort((a,b)=>b[1]-a[1]).map(([k,v])=>({"Portão":k,"Qtd":v})),{"Portão":"TOTAL","Qtd":registrosFiltrados.length}]);
      XLSX.utils.book_append_sheet(wb,ws3,"Por Portão");
      const porLider=registrosFiltrados.reduce((acc,r)=>{acc[r.lider]=(acc[r.lider]||0)+1;return acc;},{});
      const ws4=XLSX.utils.json_to_sheet([...Object.entries(porLider).sort((a,b)=>b[1]-a[1]).map(([k,v])=>({"Líder":k,"Qtd":v})),{"Líder":"TOTAL","Qtd":registrosFiltrados.length}]);
      XLSX.utils.book_append_sheet(wb,ws4,"Por Líder");
      const sufixo = dataInicio||dataFim
        ? (dataInicio?dataInicio:"inicio")+"_a_"+(dataFim?dataFim:"hoje")
        : new Date().toLocaleDateString("pt-BR").replace(/\//g,"-");
      XLSX.writeFile(wb,"relatorio_acesso_"+sufixo+".xlsx");
          ...
          <div style={{background:G.branco,borderRadius:12,border:"1.5px solid "+G.cinzaBorda,padding:"22px 20px",boxShadow:"0 2px 12px rgba(0,85,165,0.09)",animation:"fadeIn 0.3s ease"}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14}}>
              <div style={{gridColumn:"1 / -1"}}>
                ...
                <select style={inp(errors.motivo)} value={form.motivo} onChange={e=>setForm(f=>({...f,motivo:e.target.value,outrosDesc:"",descExtra:""}))}>
              ...
              {MOTIVO_DESC[form.motivo]&&(
                <div style={{gridColumn:"1 / -1",animation:"fadeIn 0.2s ease"}}>
                  <label style={lbl}>
                    {MOTIVO_DESC[form.motivo].label}
                    {MOTIVO_DESC[form.motivo].obrigatorio?" *":" (opcional)"}
                  </label>
                  <input
                    type="text"
                    placeholder={MOTIVO_DESC[form.motivo].placeholder}
                    value={form.descExtra}
                    onChange={e=>setForm(f=>({...f,descExtra:e.target.value}))}
                    style={{...inp(errors.descExtra),backgroundImage:"none",
                      fontFamily:(form.motivo==="Criança"||form.motivo==="Transferência")?"'DM Mono',monospace":"inherit",
                      letterSpacing:(form.motivo==="Criança"||form.motivo==="Transferência")?1:0}}
                  />
                  {errors.descExtra&&<div style={errStyle}>{errors.descExtra}</div>}
                </div>
              )}
              <div style={{gridColumn:"1 / -1"}}>
              ...
              <div style={{gridColumn:"1 / -1"}}>
                ...
                <div style={{display:"flex",gap:8}}>
                  {[["lider","👤 Líder"],["atendente","📡 Atendente"]].map(([val,label])=>(
                    <button key={val} onClick={()=>setForm(f=>({...f,origem:val}))}
                      style={{flex:1,padding:"10px",borderRadius:8,border:"1.5px solid "+(form.origem===val?G.azul:G.cinzaBorda),background:form.origem===val?G.azul:G.branco,color:form.origem===val?G.branco:G.cinzaTexto,fontWeight:800,fontSize:13,cursor:"pointer",fontFamily:"inherit",transition:"all 0.15s"}}>
                      {label}
                    </button>
                  ))}
                </div>
                <div style={{fontSize:10,color:G.cinzaTexto,marginTop:5}}>{form.origem==="lider"?"Você tomou a decisão de liberar":"Encaminhado por um atendente do portão"}</div>
              </div>
            </div>
            ...
            <div style={{background:G.branco,borderRadius:12,border:"1.5px solid "+G.cinzaBorda,padding:"18px 20px",marginBottom:14,boxShadow:"0 2px 12px rgba(0,85,165,0.09)"}}>
              ...
              <div style={{display:"flex",gap:10,marginTop:14,flexWrap:"wrap",alignItems:"flex-end"}}>
                <div style={{flex:1,minWidth:130}}>
                  <div style={{fontSize:11,fontWeight:800,color:G.azulEscuro,marginBottom:5,textTransform:"uppercase",letterSpacing:0.5}}>Data Início</div>
                  <input type="date" value={dataInicio} onChange={e=>setDataInicio(e.target.value)}
                    style={{width:"100%",padding:"9px 10px",borderRadius:8,border:"1.5px solid "+G.cinzaBorda,fontSize:13,fontFamily:"inherit",outline:"none",boxSizing:"border-box",color:G.preto}}/>
                </div>
                <div style={{flex:1,minWidth:130}}>
                  <div style={{fontSize:11,fontWeight:800,color:G.azulEscuro,marginBottom:5,textTransform:"uppercase",letterSpacing:0.5}}>Data Fim</div>
                  <input type="date" value={dataFim} onChange={e=>setDataFim(e.target.value)}
                    style={{width:"100%",padding:"9px 10px",borderRadius:8,border:"1.5px solid "+G.cinzaBorda,fontSize:13,fontFamily:"inherit",outline:"none",boxSizing:"border-box",color:G.preto}}/>
                </div>
                {(dataInicio||dataFim)&&(
                  <button onClick={()=>{setDataInicio("");setDataFim("");}}
                    style={{padding:"9px 14px",borderRadius:8,border:"1.5px solid "+G.cinzaBorda,background:G.branco,cursor:"pointer",fontSize:12,color:G.cinzaTexto,fontWeight:700,fontFamily:"inherit",whiteSpace:"nowrap"}}>
                    ✕ Limpar
                  </button>
                )}
              </div>
              {(dataInicio||dataFim)&&(
                <div style={{marginTop:10,fontSize:11,color:"#92400E",background:"#FEF3C7",border:"1px solid #FDE68A",borderRadius:7,padding:"7px 12px"}}>
                  📅 Exportando período: {dataInicio?new Date(dataInicio).toLocaleDateString("pt-BR"):"início"} até {dataFim?new Date(dataFim).toLocaleDateString("pt-BR"):"hoje"}
                </div>
              )}
              <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:12,marginTop:14}}>
                <div style={{fontSize:12,color:G.cinzaTexto}}>Deixe as datas em branco para exportar todos os registros</div>
