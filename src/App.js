...
export default function App(){
  ...
  const carregarRegistros=useCallback(async()=>{
    ...
    setRegistros(data.map(r=>({
      ...
      motivo:r.motivo,docCriptografado:r.cpf,docMascarado:"****",origem:r.origem||"lider",
    })));
    setLoading(false);
  },[]);
  ...
  const handleSubmit=async()=>{
    ...
    try{
      ...
      const {error}=await supabase.from("atendimentos").insert([{
        cpf:docCriptografado,motivo:motivoFinal+(form.descExtra.trim()?" | "+form.descExtra.trim():""),portao:form.portao,lider:form.lider,hora,data,origem:form.origem
      }]);
  ...
  const handleExportarComAdmin=async(adminPin)=>{
    ...
    try{
      ...
      const ws1=XLSX.utils.json_to_sheet(registrosFiltrados.map((r,i)=>({
        "#":i+1,"Data":r.data,"Hora":r.hora,"DOC":docsDescriptografados[i],"Origem":r.origem==="atendente"?"Atendente":"Líder",
