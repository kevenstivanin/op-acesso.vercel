...
export default function App(){
  ...
  const [adminPinHistorico,setAdminPinHistorico]=useState("");
  const [showPinHistorico,setShowPinHistorico]=useState(false);
  const [docParaBuscar,setDocParaBuscar]=useState("");
  ...
  const buscarHistorico=async(adminPin)=>{
    setShowPinHistorico(false);
    setBuscando(true);
    setHistorico(null);
    try{
      const docBusca = docParaBuscar.trim();
      const encontrados=[];
      for(const r of registros){
        try{
          const dec=await descriptografarDoc(r.docCriptografado, adminPin);
  ...
  const TABS=[["form","✏️ Registrar"],["lista","📋 Registros ("+registros.length+")"],["historico","🔍 Histórico"],["relatorio","📊 Relatório"]];
  ...
  return(
    <div style={{minHeight:"100vh",background:G.cinzaClaro,fontFamily:"'Barlow','Segoe UI',sans-serif"}}>
      ...
      {showPinHistorico&&<ModalAdminPin onConfirm={buscarHistorico} onCancel={()=>setShowPinHistorico(false)}/>}
          ...
          <div style={{animation:"fadeIn 0.3s ease"}}>
            <div style={{background:G.branco,borderRadius:12,border:"1.5px solid "+G.cinzaBorda,padding:"20px",marginBottom:14,boxShadow:"0 2px 12px rgba(0,85,165,0.09)"}}>
              ...
              <div style={{display:"flex",gap:10}}>
                ...
                <button
                  onClick={()=>{ if(!buscaDoc.trim()) return; setDocParaBuscar(buscaDoc); setShowPinHistorico(true); }}
